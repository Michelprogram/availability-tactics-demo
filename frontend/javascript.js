...
async function pollHealth(){
  try{
    ...
    setBadge(activeBadge, lastActiveUrl ? 
      (lastActiveUrl.includes("backend-primary") ? "PRIMARY" : "BACKUP") : "ACTIF", 
      lastActiveUrl?.includes("backend-primary") ? "primary" : 
      lastActiveUrl?.includes("backend-spare") ? "backup" : "neutral"
    );
    ...
    if(lastActiveUrl?.includes("backend-primary")){
      ...
      bakCode.textContent = "—"; bakLat.textContent = "—";
    }
  }catch(e){
...
async function doData(){
  ...
  try{
    ...
    addRow({action:"GET /api/data", code:res.status, servedBy, latency:dt});
  }catch(err){
    const dt = Math.max(1, Math.round(performance.now() - t0));
    addRow({action:"GET /api/data", code:"ERR", latency:dt, detail:String(err?.message || err)});
  }
}
